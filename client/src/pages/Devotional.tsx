...
export default function Devotional() {
  ...
  return <AppShell><div className="mx-auto max-w-3xl px-4 py-7 sm:px-7 lg:py-12"><div className="mb-7 flex items-center justify-between"><Button variant="ghost" onClick={() => navigate("/")} className="-ml-3 gap-2 text-[#66756c]"><ArrowLeft className="h-4 w-4" /> Hoje</Button><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#a2813c]">Minuto da Provisão</p><p className="mt-0.5 text-xs text-[#78857d]">Dia {dayNumber} de 365</p></div></div>
    <article className="rounded-[28px] border border-[#e6e0d3] bg-[#fffdfa] px-6 py-8 shadow-[0_10px_35px_rgba(22,39,29,.05)] dark:border-white/10 dark:bg-[#17231f] sm:px-10 sm:py-11"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#b38c31]">{devotional.theme} · {devotional.month}</p><h1 className="mt-3 font-serif text-4xl font-semibold leading-[1.05] tracking-tight sm:text-5xl">{devotional.title}</h1></div><button aria-label="Favoritar devocional" onClick={handleFavorite} className={isFavorite ? "flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#f9e8e5] text-[#b85b4d]" : "flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#f2f1eb] text-[#7c887f] hover:text-[#b85b4d] dark:bg-white/5"}><Heart className={isFavorite ? "h-5 w-5 fill-current" : "h-5 w-5"} /></button></div>
      <div className="mt-8 border-y border-[#ece6d9] py-5 dark:border-white/10"><p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.16em] text-[#9a7730]"><BookMarked className="h-3.5 w-3.5" /> Referência bíblica · {bibleTranslation.shortLabel}</p><p className="mt-2 font-serif text-2xl text-[#274432] dark:text-[#eadcaa]">{devotional.bibleReference}</p><p className="mt-2 text-xs leading-5 text-[#79867d]">Tradução: {bibleTranslation.label}. {bibleTranslation.license}. O texto integral poderá ser conectado a esta fonte sem alterar os devocionais ou o aplicativo.</p></div>
      <section aria-label="Áudio do devocional" className="mt-8 rounded-2xl border border-[#ded7c8] bg-[#f7f3e8] p-5 dark:border-white/10 dark:bg-[#21342b]"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#d9b45e] text-[#102820]"><Volume2 className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8d6e2d] dark:text-[#e6d099]">Ouvir este devocional</p><p className="mt-1 text-sm leading-6 text-[#637169] dark:text-[#c4d1c8]">{generatedAudioUrl ? "Locução masculina brasileira, natural e conversacional." : "Leitura em voz clara, suave e pausada para acompanhar sua reflexão."}</p>{generatedAudioUrl ? <audio className="mt-4 w-full" controls preload="metadata" src={generatedAudioUrl}><track kind="captions" /></audio> : <div className="mt-4 flex flex-wrap gap-2"><Button onClick={isSpeaking ? pauseAudio : playAudio} className="rounded-xl bg-[#102820] text-white hover:bg-[#1d4938]"><span className="mr-2">{isSpeaking ? (isPaused ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />) : <Play className="h-4 w-4" />}</span>{isSpeaking ? (isPaused ? "Continuar" : "Pausar") : "Dar play no áudio"}</Button>{isSpeaking && <Button variant="outline" onClick={stopAudio} className="rounded-xl border-[#cbbd9e] text-[#70591e] dark:text-[#e6d099]"><Square className="mr-2 h-3.5 w-3.5 fill-current" /> Parar</Button>}</div>}</div></div></section>
