...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS subscriptions (id int AUTO_INCREMENT NOT NULL, userId int NOT NULL, planId varchar(16) NOT NULL DEFAULT 'free', status varchar(24) NOT NULL DEFAULT 'trialing', stripeCustomerId varchar(120), stripeSubscriptionId varchar(120), stripePriceId varchar(120), currentPeriodStart timestamp NULL, currentPeriodEnd timestamp NULL, trialStart timestamp NULL, trialEnd timestamp NULL, cancelAtPeriodEnd boolean NOT NULL DEFAULT false, canceledAt timestamp NULL, createdAt timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, updatedAt timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, CONSTRAINT subscriptions_id PRIMARY KEY (id), CONSTRAINT subscriptions_user_unique UNIQUE (userId), CONSTRAINT subscriptions_stripe_subscription_unique UNIQUE (stripeSubscriptionId), CONSTRAINT subscriptions_userId_fk FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE)`,
  `CREATE TABLE IF NOT EXISTS payment_audit_logs (id int AUTO_INCREMENT NOT NULL, userId int NULL, stripeEventId varchar(120) NOT NULL, eventType varchar(100) NOT NULL, payload text NULL, createdAt timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT payment_audit_logs_id PRIMARY KEY (id), CONSTRAINT payment_audit_logs_stripeEventId_unique UNIQUE (stripeEventId), CONSTRAINT payment_audit_logs_userId_fk FOREIGN KEY (userId) REFERENCES users(id) ON DELETE SET NULL)`,
